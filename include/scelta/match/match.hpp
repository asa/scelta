...
struct return_type<deduce_rt> {
    ...
    using apply = std::invoke_result<F, Ts...>;
};

// SFINAE-friendly return type dispatch.
// Alias for `return_type<Return>::apply<F, Ts...>`.
template <typename Return, typename F, typename... Ts>
using applied_return_type = typename return_type<Return>::template apply<F, Ts...>;

template <typename T>
using original_decay_t = ::scelta::recursive::impl::original_type_t<std::decay_t<T>>;

template <typename T>
using original_decay_first_alternative_t = ::scelta::traits::adt::first_alternative<original_decay_t<T>>;

template <typename O>
struct unresolved_visitor : O {
    template <typename OFwd>
    constexpr unresolved_visitor(OFwd&& o) noexcept(noexcept(O{FWD(o)})) : O{FWD(o)} {}
...
constexpr auto make_unresolved_visitor(BCO&& bco) SCELTA_RETURNS(unresolved_visitor<std::decay_t<BCO>>{FWD(bco)})
    ...
    struct with_bound_base_cases : private BCO  // "base case overload", EBO
{
   ...
   private:
    ...
    constexpr auto do_non_recursive(Xs&&... xs)
        SCELTA_NOEXCEPT_AND_TRT(::scelta::nonrecursive::visit(std::declval<BCO&>(), FWD(xs)...)) {
    ...
    constexpr auto defer_recursive(Xs&&... xs) {
        ...
        auto adapted_bco = [bco = static_cast<BCO&&>(*this)]  //
            ...
            SCELTA_NOEXCEPT_AND_TRT(std::declval<BCO&&>()(FWD(ys)...)) { return bco(FWD(ys)...); };

        // Overload of "adapted BCO" and "recursive cases".
        auto o = overload(std::move(adapted_bco), FWD(xs)...);

        return [urv = make_unresolved_visitor(std::move(o))](auto&&... vs) mutable -> decltype(auto) {
            using rt = applied_return_type<Return, BCO&, original_decay_first_alternative_t<decltype(vs)>...>;
